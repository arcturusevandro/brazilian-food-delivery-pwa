...
function MenuShell() {
  ...
  return (
    <>
      <div className="min-h-dvh bg-background pb-28">
        ...
        {!restaurant.is_open && (
          <div className="bg-destructive/10 border-b border-destructive/20 px-4 py-3 text-center">
            <p className="text-sm font-medium text-destructive">🔴 Estamos fechados no momento — aceitaremos pedidos em breve!</p>
          </div>
        )}
        ...
        {cart.length > 0 && !checkoutOpen && !detailProduct && !orderConfirmed && restaurant.is_open && (
          <div className="fixed bottom-0 left-0 right-0 z-40 px-4 pb-4 pt-2 bg-gradient-to-t from-background via-background/95 to-transparent">
            <button onClick={() => setCartOpen(true)} className="w-full flex items-center justify-between gap-3 rounded-2xl bg-primary px-5 py-4 text-primary-foreground shadow-xl active:scale-[0.98] transition-all duration-150">
              <div className="flex items-center gap-2"><div className="flex h-7 w-7 items-center justify-center rounded-full bg-primary-foreground/20 font-bold text-sm">{cartItemCount}</div><span className="font-semibold text-sm">Ver carrinho</span></div>
      ...
      {detailProduct && <ProductDetailPanel product={detailProduct} addons={addonsByProduct.get(detailProduct.id) || []} onClose={() => setDetailProduct(null)} onConfirm={addons => { addWithAddons(detailProduct, addons); setDetailProduct(null) }} />}
      {cartOpen && <CartPanel cart={cart} cartSubtotal={cartSubtotal} cartItemCount={cartItemCount} restaurantName={restaurant.name} onIncrement={increment} onDecrement={decrement} onRemove={removeItem} onClose={() => setCartOpen(false)} onCheckout={() => { setCartOpen(false); setCheckoutOpen(true) }} />}
      {checkoutOpen && <CheckoutPanel cart={cart} cartSubtotal={cartSubtotal} cartItemCount={cartItemCount} restaurantName={restaurant.name} submitting={submitting} deliverySettings={deliverySettings} deliveryZones={deliveryZones} onSubmit={submitOrder} onClose={() => setCheckoutOpen(false)} onBack={() => { setCheckoutOpen(false); setCartOpen(true) }} />}
      {orderConfirmed && <OrderConfirmationPanel total={orderConfirmed.total} deliveryFee={orderConfirmed.deliveryFee} onClose={() => setOrderConfirmed(null)} />}
    </>
  )
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      <div className="relative flex flex-col max-h-[85dvh] rounded-t-2xl bg-card shadow-2xl animate-slide-up border-t border-border">
        <div className="flex-shrink-0 px-4 pt-3 pb-2 border-b border-border"><div className="mx-auto mb-2 h-1.5 w-10 rounded-full bg-muted" /><div className="flex items-center justify-between"><h2 className="text-lg font-semibold">{product.name}</h2><button onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-full bg-muted hover:bg-accent transition-colors"><X className="h-4 w-4" /></button></div></div>
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          ...
          <div className="space-y-2"><h3 className="text-sm font-semibold">Adicionais <span className="text-muted-foreground font-normal">(opcional)</span></h3>{addons.map(a => <button key={a.id} type="button" onClick={() => toggle(a.id)} className={`w-full flex items-center justify-between gap-3 rounded-lg border px-3 py-2.5 transition-all ${selected.has(a.id) ? 'border-primary bg-primary/10' : 'border-border bg-background hover:border-primary/40'}`}><div className="flex items-center gap-2.5"><div className={`flex h-5 w-5 items-center justify-center rounded border-2 transition-colors ${selected.has(a.id) ? 'border-primary bg-primary text-primary-foreground' : 'border-muted-foreground/40'}`}>{selected.has(a.id) && <Plus className="h-3 w-3 rotate-45" />}</div><span className="text-sm font-medium">{a.name}</span></div><span className="text-sm font-semibold text-primary">{a.price > 0 ? `+ ${formatBRL(a.price)}` : 'Grátis'}</span></button>)}</div>
        </div>
        <div className="flex-shrink-0 border-t border-border px-4 py-3 bg-card"><Button onClick={() => onConfirm(sel)} className="w-full h-12 text-base font-semibold">Adicionar · {formatBRL(total)}</Button></div>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      <div className="relative flex flex-col max-h-[80dvh] rounded-t-2xl bg-card shadow-2xl animate-slide-up border-t border-border">
        <div className="flex-shrink-0 px-4 pt-3 pb-2 border-b border-border"><div className="mx-auto mb-2 h-1.5 w-10 rounded-full bg-muted" /><div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold">Seu carrinho</h2><p className="text-xs text-muted-foreground">{cartItemCount} {cartItemCount === 1 ? 'item' : 'itens'} · {restaurantName}</p></div><button onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-full bg-muted hover:bg-accent transition-colors"><X className="h-4 w-4" /></button></div></div>
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">{cart.map((item: CartItem) => <div key={item.cartId} className="flex items-start gap-3 rounded-lg border border-border bg-background p-2.5"><div className="flex-shrink-0 h-10 w-10 rounded-lg bg-accent flex items-center justify-center text-lg">{getFoodEmoji(item.product.name)}</div><div className="flex-1 min-w-0"><p className="text-sm font-medium truncate">{item.product.name}</p>{item.addons.length > 0 && <p className="text-xs text-muted-foreground">+ {item.addons.map((a: Addon) => a.name).join(', ')}</p>}<p className="text-xs text-muted-foreground mt-0.5">{formatBRL(itemUnitPrice(item))} cada</p></div><div className="flex flex-col items-end gap-1.5"><div className="flex items-center gap-1.5"><button onClick={() => onDecrement(item.cartId)} className="flex h-6 w-6 items-center justify-center rounded-full bg-muted hover:bg-primary hover:text-primary-foreground transition-colors"><Minus className="h-3 w-3" /></button><span className="w-5 text-center text-sm font-semibold">{item.quantity}</span><button onClick={() => onIncrement(item.cartId)} className="flex h-6 w-6 items-center justify-center rounded-full bg-primary text-primary-foreground hover:bg-primary/90 transition-colors active:scale-90"><Plus className="h-3 w-3" /></button></div><span className="text-sm font-semibold">{formatBRL(itemUnitPrice(item) * item.quantity)}</span></div></div>)}</div>
        <div className="flex-shrink-0 border-t border-border px-4 py-3 space-y-3 bg-card"><div className="flex items-center justify-between"><span className="text-sm text-muted-foreground">Subtotal</span><span className="text-lg font-bold">{formatBRL(cartSubtotal)}</span></div><Button onClick={onCheckout} className="w-full h-12 text-base font-semibold gap-2"><ShoppingBag className="h-5 w-5" />Ir para o checkout</Button></div>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      <div className="relative flex flex-col max-h-[90dvh] rounded-t-2xl bg-card shadow-2xl animate-slide-up border-t border-border">
        <div className="flex-shrink-0 px-4 pt-3 pb-2 border-b border-border"><div className="mx-auto mb-2 h-1.5 w-10 rounded-full bg-muted" /><div className="flex items-center justify-between"><div className="flex items-center gap-2"><button onClick={onBack} className="flex h-8 w-8 items-center justify-center rounded-full bg-muted hover:bg-accent transition-colors"><ArrowLeft className="h-4 w-4" /></button><div><h2 className="text-lg font-semibold">Checkout</h2><p className="text-xs text-muted-foreground">{cartItemCount} itens · {formatBRL(total)}</p></div></div><button onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-full bg-muted hover:bg-accent transition-colors"><X className="h-4 w-4" /></button></div></div>
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          <div className="rounded-lg border border-border bg-background p-3 space-y-1.5"><p className="text-xs font-medium text-muted-foreground">Resumo</p>{cart.map((item: CartItem) => <div key={item.cartId} className="flex items-start justify-between text-sm gap-2"><span className="flex-1">{item.quantity}x {item.product.name}{item.addons.length > 0 && <span className="block text-xs text-muted-foreground">+ {item.addons.map((a: Addon) => a.name).join(', ')}</span>}</span><span className="text-muted-foreground shrink-0">{formatBRL(itemUnitPrice(item) * item.quantity)}</span></div>)}<div className="border-t border-border pt-1.5 space-y-1"><div className="flex justify-between text-sm"><span className="text-muted-foreground">Subtotal</span><span>{formatBRL(cartSubtotal)}</span></div><div className="flex justify-between text-sm"><span className="text-muted-foreground">Entrega</span><span className={deliveryFee === 0 ? 'text-green-600 font-medium' : ''}>{deliveryFee === 0 ? 'Grátis' : formatBRL(deliveryFee)}</span></div><div className="flex justify-between font-bold pt-1 border-t border-border"><span className="text-sm">Total</span><span className="text-sm text-primary">{formatBRL(total)}</span></div></div></div>
          <div className="space-y-1.5"><label className="text-sm font-medium">Nome *</label><Input value={form.customer_name} onChange={e => setForm(f => ({ ...f, customer_name: e.target.value }))} placeholder="Seu nome completo" className={errors.customer_name ? 'border-destructive' : ''} />{errors.customer_name && <p className="text-xs text-destructive">{errors.customer_name}</p>}</div>
          <div className="space-y-1.5"><label className="text-sm font-medium">Telefone *</label><Input value={form.customer_phone} onChange={e => setForm(f => ({ ...f, customer_phone: e.target.value }))} placeholder="(66) 99999-9999" type="tel" className={errors.customer_phone ? 'border-destructive' : ''} />{errors.customer_phone && <p className="text-xs text-destructive">{errors.customer_phone}</p>}</div>
          {deliverySettings.type === 'by_neighborhood' && <div className="space-y-1.5"><label className="text-sm font-medium">Bairro *</label><select value={form.neighborhood} onChange={e => setForm(f => ({ ...f, neighborhood: e.target.value }))} className={`w-full h-10 rounded-md border px-3 text-sm bg-background text-foreground ${errors.neighborhood ? 'border-destructive' : 'border-input'}`}><option value="">Selecione seu bairro...</option>{deliveryZones.map((z: DeliveryZone) => <option key={z.id} value={z.neighborhood}>{z.neighborhood} — {z.fee > 0 ? formatBRL(z.fee) : 'Grátis'}</option>)}</select>{errors.neighborhood && <p className="text-xs text-destructive">{errors.neighborhood}</p>}</div>}
          <div className="space-y-1.5"><label className="text-sm font-medium">Endereço *</label><Input value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))} placeholder="Rua, número" className={errors.address ? 'border-destructive' : ''} />{errors.address && <p className="text-xs text-destructive">{errors.address}</p>}</div>
          <div className="space-y-1.5"><label className="text-sm font-medium">Forma de pagamento</label><div className="grid grid-cols-3 gap-2">{[{ v: 'cash', l: 'Dinheiro', e: '💵' }, { v: 'card', l: 'Cartão', e: '💳' }, { v: 'pix', l: 'Pix', e: '⚡' }].map(opt => <button key={opt.v} type="button" onClick={() => setForm(f => ({ ...f, payment_method: opt.v as PaymentMethod, change_for: '' }))} className={`flex flex-col items-center justify-center gap-1 rounded-lg border px-2 py-2.5 text-xs font-medium transition-all ${form.payment_method === opt.v ? 'border-primary bg-primary/10 text-primary' : 'border-border bg-background text-muted-foreground hover:border-primary/40'}`}><span className="text-xl">{opt.e}</span>{opt.l}</button>)}</div>{form.payment_method === 'cash' && <div className="mt-2 space-y-1.5"><label className="text-sm font-medium">Troco para quanto? <span className="text-muted-foreground font-normal">(opcional)</span></label><Input value={form.change_for} onChange={e => setForm(f => ({ ...f, change_for: e.target.value }))} placeholder="Ex: 50,00" type="number" inputMode="decimal" className={errors.change_for ? 'border-destructive' : ''} />{errors.change_for && <p className="text-xs text-destructive">{errors.change_for}</p>}{!errors.change_for && <p className="text-xs text-muted-foreground">Deixe em branco se não precisar de troco</p>}</div>}</div>
          <div className="space-y-1.5"><label className="text-sm font-medium">Observações</label><Input value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} placeholder="Ex: sem cebola, sem molho..." /></div>
          <div className="rounded-lg bg-accent px-3 py-2.5 flex items-start gap-2"><MapPin className="h-4 w-4 text-primary flex-shrink-0 mt-0.5" /><p className="text-xs text-accent-foreground">Entrega em aproximadamente 40-60 minutos.</p></div>
        </div>
        <div className="flex-shrink-0 border-t border-border px-4 py-3 bg-card"><Button onClick={() => { if (validate()) onSubmit(form, deliveryFee) }} disabled={submitting} className="w-full h-12 text-base font-semibold">{submitting ? 'Enviando pedido...' : `Confirmar pedido · ${formatBRL(total)}`}</Button></div>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-sm rounded-2xl bg-card shadow-2xl p-6 flex flex-col items-center gap-4 animate-fade-in">
        <div className="flex h-16 w-16 items-center justify-center rounded-full bg-green-100"><svg className="h-8 w-8 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg></div>
        <div className="text-center space-y-1"><h2 className="text-xl font-bold">Pedido realizado!</h2><p className="text-sm text-muted-foreground">Seu pedido foi recebido com sucesso</p></div>
        <div className="w-full rounded-xl bg-muted/50 p-4 space-y-2"><div className="flex justify-between text-sm text-muted-foreground"><span>Subtotal</span><span>{formatBRL(subtotal)}</span></div><div className="flex justify-between text-sm text-muted-foreground"><span>Entrega</span><span>{deliveryFee > 0 ? formatBRL(deliveryFee) : 'Grátis'}</span></div><div className="flex justify-between font-bold text-foreground border-t border-border pt-2"><span>Total</span><span className="text-primary">{formatBRL(total)}</span></div></div>
        <div className="flex items-center gap-2 text-xs text-muted-foreground text-center"><Clock className="h-3.5 w-3.5 shrink-0" /><span>Entrega em aproximadamente 40-60 minutos</span></div>
